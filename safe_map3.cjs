...
for (const [ru, en] of dict) {
  content = content.replace(new RegExp(">" + ru + "<", "g"), ">{i18n.language === 'en' ? '" + en.replace(/'/g, "\\\\'") + "' : '" + ru.replace(/'/g, "\\\\'") + "'}<");
  
  if (!content.includes(ru)) continue;
  content = content.replace(ru, "{i18n.language === 'en' ? '" + en.replace(/'/g, "\\\\'") + "' : '" + ru.replace(/'/g, "\\\\'") + "'}");
}
